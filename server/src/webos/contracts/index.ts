...
function asIssues(ok: boolean, issues: ContractIssue[]): ContractResult {
  return { ok, issues }
}
...
/**
 * 校验 daily.pkg.json（schema 结构 + 语义）。返回 ok + issues（人话）。
...
export function validatePackageManifest(raw: unknown, depth = 1): ContractResult {
  ...
  if (!schemaOk) {
    ...
    if (schemaIssues.length === 0) {
      schemaIssues.push({ path: 'schema', message: 'manifest 结构与 schema 不符，请对照 docs/routes/web/03-package-system.md §2 逐字段检查' })
    }
