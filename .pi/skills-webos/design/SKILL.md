---
name: design
description: 花叔Design——用HTML做高保真原型、幻灯片、动画、可视化与专家评审。任何新设计100%先出三个方向初稿给用户选（指定风格/品牌也不豁免），选定后才执行。触发词：做原型、PPT、幻灯片、动画、设计风格、评审、做个HTML页面、UI mockup、导出MP4/GIF、做个好看的。生产级Web App/需后端的系统不适用。附带 Open Design 精选设计系统（design-systems/，如 apple/linear-app/stripe 等，按需读取）。
---
...
## Open Design 设计系统库（本仓库追加，不改动上述任何规则）

本 skill 附带 **Open Design 精选设计系统**，放在本 skill 根目录的 `design-systems/` 下（每套 = `DESIGN.md` 设计语言 + `tokens.css` 设计 Token + `components.html` 组件预览）：

```
design-systems/
├── README.md          # Open Design 全量索引（151 套，服务器未全量部署）
├── apple/             # Apple 设计语言
├── linear-app/        # Linear 设计系统
├── stripe/  notion/  material/  claude/  airbnb/  spotify/  shopify/
├── glassmorphism/  neumorphism/  minimal/  brutalism/  clay/
├── bento/  arc/  cafe/  colorful/  clean/
```

**用法**：
- 用户指定了某品牌/风格（「像苹果」「Linear 风」「玻璃拟态」「极简」）→ read `design-systems/<name>/DESIGN.md` 获取设计语言；需要具体色值时 read 同目录 `tokens.css`；组件细节参考 `components.html`。
- 用户没有具体指定 → 按上述 Huashu 流程自由设计，需要风格弹药时也可从 `design-systems/` 与 `references/design-styles.md` 取材。
- 两者是互补关系：Huashu 提供流程与方法论（三方向、反 slop、资产协议），Open Design 提供风格与组件参考——**最终都以用户需求为准**。

---

## webOS 对话环境适配（本仓库追加）：三方向选择 = 对话内互动 HTML

Daily 平台提供两个能力，让「三方向给用户选」在对话里直接闭环，用户不用打字：

1. **show_interactive_html**：把自包含 HTML 组件插入对话（sandbox iframe 渲染，宽 100%、高 120-480px、建议 220-320px）；
2. **互动答案回传**：组件内按钮点击时执行
   `parent.postMessage({channel:"daily-webos-sdk",kind:"event",payload:{type:"interactive_answer",value:"用户点击的答案"}},"*")`
   ——宿主会把 value 作为用户的下一条消息自动发回给你。

**所以「三方向初稿给用户选」在 Daily 对话里这样落地**：
1. 按上述流程产出三个方向的真实初稿（HTML 或方向板）；
2. 用 `show_interactive_html` 插入**方向选择器**：三个横向卡片（方向名 + 一句话气质定位 + 色板/布局示意），每张卡片整卡可点；
3. 用户点击 → 你收到 `interactive_answer`（value 如 "方向2" 或 "方向1配色+方向3布局"）→ 按选定方向继续执行。

选择器示例结构（自包含、无外部依赖、高度建议 240-320px）：
```html
<div style="font-family:system-ui;padding:4px 2px">
  <div style="font-size:13px;font-weight:700;margin-bottom:8px">选择你喜欢的风格方向：</div>
  <div style="display:flex;gap:8px">
    <button onclick="pick('方向1 · 静谧极简')" style="flex:1;border:1px solid #e2e0d8;border-radius:14px;padding:10px 8px;background:#fff;text-align:left;cursor:pointer">
      <div style="font-size:12px;font-weight:700">方向1 · 静谧极简</div>
      <div style="display:flex;gap:3px;margin:6px 0"><span style="flex:1;height:6px;border-radius:3px;background:#f8f7f3"></span><span style="flex:1;height:6px;border-radius:3px;background:#171918"></span><span style="flex:1;height:6px;border-radius:3px;background:#315bd6"></span></div>
      <div style="font-size:10px;color:#71756f;line-height:1.5">大留白 · 纸感 · 靛蓝点缀</div>
    </button>
    <button onclick="pick('方向2 · 深色沉浸')" style="...">...</button>
    <button onclick="pick('方向3 · 玻璃拟态')" style="...">...</button>
  </div>
</div>
<script>
  function pick(v){ parent.postMessage({channel:"daily-webos-sdk",kind:"event",payload:{type:"interactive_answer",value:v}},"*") }
</script>
```

其他需要「用户即时拍板」的场合（确认采用哪个方案、选配色、选布局、A/B 对比）也一律优先用这个交互，不要让用户打字描述。
