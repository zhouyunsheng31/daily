...
- **工具管理**：列出/启用/禁用 30 个工具（高级，含 ask_user + query_capabilities）

---

## 六、AI 工具列表（共 30 个）

共 30 个工具（4 Widget + 2 Storage + 18 Browser + 1 ask_user + 4 Search + 1 System）。
...
| 工具 | 功能 |
|------|------|
| ask_user | AI 主动向用户提问（选项框形式，支持多选），不走 executeToolCall，通过 useAIStore 弹 AskUserCard |
...
| 工具 | 功能 |
|------|------|
| query_capabilities | 查询所有组件的能力声明（widgetType / displayName / description / api / dependencies），AI 用于发现并调用其他组件 |
...
# 4. 运行 vitest E2E 占位测试
npx vitest run e2e/phase11-e2e.spec.ts
```

### 9.3 测试集参考

详见 [ai-assistant-testsets.md](../../docs/superpowers/testsets/ai-assistant-testsets.md)，覆盖 25 工具的 E2E 行为测试。
