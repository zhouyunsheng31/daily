...
const ALLOWED_COMMANDS = new Set<string>(['ps', 'up', 'down', 'logs', 'run', 'exec']);
...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--file' || arg === '-f') {
      ...
      result.files.push(argv[++i] as string);
      continue;
    }
...
function runDocker(args: string[]): Promise<ExecResult> {
  return new Promise((resolveFn) => {
    execFile(
      'docker',
      args,
      { shell: false, maxBuffer: 50 * 1024 * 1024 },
      (err, stdout, stderr) => {
        if (err) {
 ...
 */
async function resolveNetworkName(baseName: string): Promise<string> {
...
function emitSuccess(data: unknown, json: boolean): void {
  if (json) {
...
function emitError(error: string, json: boolean, exitCode: number): never {
  if (json) {
...
async function cmdPs(json: boolean): Promise<number> {
  ...
  for (const line of r.stdout.split('\n')) {
    ...
    if (!trimmed) continue;
    try {
      containers.push(JSON.parse(trimmed) as Record<string, string>);
    } catch {
  ...
  if (json) {
    ...
    if (containers.length === 0) {
      ...
      const headers = ['Name', 'Image', 'Status', 'Ports'] as const;
      const widths = headers.map(
        (h) => Math.max(h.length, ...rows.map((row) => String(row[h]).length))
      );
      const fmt = (vals: readonly string[]): string =>
        vals.map((v, i) => String(v).padEnd(widths[i] as number)).join('  ');
...
async function cmdUp(service: string, files: string[], json: boolean): Promise<number> {
  const args: string[] = ['compose', '-f', MAIN_COMPOSE_FILE];
  ...
  if (r.exitCode !== 0) {
    emitError(
      r.stderr.trim() || `docker compose up ${service} failed`,
      json,
      EXIT_BUSINESS_ERROR
    );
  }
  ...
  if (json) {
    emitSuccess(
      { service, action: 'up', stdout: r.stdout, stderr: r.stderr },
      json
    );
  } else {
...
async function cmdDown(service: string, files: string[], json: boolean): Promise<number> {
  const args: string[] = ['compose', '-f', MAIN_COMPOSE_FILE];
  for (const f of files) {
    args.push('-f', f);
  }
  ...
  if (r.exitCode !== 0) {
    emitError(
      r.stderr.trim() || `docker compose down ${service} failed`,
      json,
      EXIT_BUSINESS_ERROR
    );
  }
  ...
  if (json) {
    emitSuccess(
      { service, action: 'down', stdout: r.stdout, stderr: r.stderr },
      json
    );
  } else {
    process.stdout.write(r.stdout);
...
async function cmdLogs(
  ...
  if (r.exitCode !== 0) {
    emitError(
      r.stderr.trim() || `docker compose logs ${service} failed`,
      json,
      EXIT_BUSINESS_ERROR
    );
  }
...
async function cmdRun(image: string, cmd: string[], json: boolean): Promise<number> {
  ...
  if (r.exitCode !== 0) {
    emitError(
      r.stderr.trim() || `docker run ${image} failed`,
      json,
      EXIT_BUSINESS_ERROR
    );
  }
  ...
  if (json) {
    emitSuccess(
      { image, cmd, stdout: r.stdout, stderr: r.stderr, exitCode: r.exitCode },
      json
    );
  } else {
    process.stdout.write(r.stdout);
...
async function cmdExec(
  ...
  const args: string[] = ['compose', 'exec', service, ...cmd];

  const r = await runDocker(args);
  if (r.exitCode !== 0) {
    emitError(
      r.stderr.trim() || `docker compose exec ${service} failed`,
      json,
      EXIT_BUSINESS_ERROR
    );
  }
  ...
  if (json) {
    emitSuccess(
      { service, cmd, stdout: r.stdout, stderr: r.stderr, exitCode: r.exitCode },
      json
    );
  } else {
    process.stdout.write(r.stdout);
...
async function main(): Promise<number> {
  ...
  if (!ALLOWED_COMMANDS.has(parsed.command)) {
    emitError(
      `Unknown command: ${parsed.command}. Allowed: ps, up, down, logs, run, exec`,
      jsonFlag,
      EXIT_PARAM_ERROR
    );
  }
  ...
  switch (parsed.command) {
    ...
    case 'up': {
      ...
      if (!service) {
        emitError(
          'Usage: up <service> [--file <overlay>] [--json]',
          jsonFlag,
          EXIT_PARAM_ERROR
        );
      }
    ...
    case 'down': {
      ...
      if (!service) {
        emitError(
          'Usage: down <service> [--file <overlay>] [--json]',
          jsonFlag,
          EXIT_PARAM_ERROR
        );
      }
    ...
    case 'logs': {
      ...
      if (!service) {
        emitError(
          'Usage: logs <service> [--tail <n>] [--json]',
          jsonFlag,
          EXIT_PARAM_ERROR
        );
      }
    ...
    case 'exec': {
      ...
      if (cmd.length === 0) {
        emitError(
          'Usage: exec <service> <cmd...> [--json] — cmd is required',
          jsonFlag,
          EXIT_PARAM_ERROR
        );
      }
