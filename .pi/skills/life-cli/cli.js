...
async function listEntities(type, limit) {
    const resp = (await apiFetch(`/api/entities?type=${type}&limit=${limit}`));
    return resp.items;
}
...
async function habitCheckin(habitId, json) {
...
// ===== Mood Commands =====
async function moodAdd(score, note, json) {
...
async function moodHistory(limit, json) {
...
// ===== Focus Commands =====
async function focusStart(goal, json) {
    const data = { action: 'start', startTime: Date.now() };
    if (goal)
        data.goal = goal;
...
async function focusStats(json) {
...
async function savingsCreate(name, target, json) {
...
async function savingsUpdate(goalId, amount, json) {
...
// ===== QuickNote Commands =====
async function quicknoteAdd(content, json) {
...
async function quicknoteLs(limit, json) {
...
async function quicknoteSearch(query, json) {
    const items = await listEntities('quickNote', 1000);
    const q = query.toLowerCase();
    const matches = items.filter((e) => JSON.stringify(e.data || {}).toLowerCase().includes(q));
    if (json) {
...
// ===== Main =====
const USAGE = 'Usage: life-cli <resource> <command> [args] [--json]\n' +
    'Resources & Commands:\n' +
    '  habit ls [--json]\n' +
    '  habit checkin --id <habitId> [--json]\n' +
    '  mood add --score <n> [--note <text>] [--json]\n' +
    ...
    try {
        if (resource === 'habit') {
            ...
            if (cmd === 'ls') {
                ...
                const id = flags['id'];
                if (!id)
                    fail('Usage: habit checkin --id <habitId> [--json]', json, 2);
            ...
            if (cmd === 'add') {
                ...
                if (!scoreStr)
                    fail('Usage: mood add --score <n> [--note <text>] [--json]', json, 2);
                const score = parseInt(scoreStr, 10);
                if (isNaN(score))
                    fail(`Invalid score: ${scoreStr}`, json, 2);
            ...
            if (cmd === 'ls') {
                ...
                if (isNaN(target))
                    fail(`Invalid target amount: ${targetStr}`, json, 2);
                ...
                if (isNaN(amount))
                    fail(`Invalid amount: ${amountStr}`, json, 2);
            ...
            if (cmd === 'add') {
                ...
                if (!content)
                    fail('Usage: quicknote add --content <text> [--json]', json, 2);
                ...
                if (!query)
                    fail('Usage: quicknote search <query> [--json]', json, 2);
                await quicknoteSearch(query, json);
            }
            else {
                fail(`Unknown quicknote command: ${cmd || '(none)'}\nAvailable: add, ls, search`, json, 2);
            }
        }
        else {
            fail(`Unknown resource: ${resource}\nAvailable: habit, mood, focus, savings, quicknote`, json, 2);
        }
    }
    catch (err) {
