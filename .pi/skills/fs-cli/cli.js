...
import { realpath, readdir, stat as fsStat, readFile, writeFile, mkdir, rm as fsRm, rename, copyFile, appendFile, stat, } from 'node:fs/promises';
// ============ Constants ============
const WHITELIST = [
...
// ============ Audit Log ============
async function writeAuditLog(operation, path, result) {
    try {
...
async function moveSafe(src, dst) {
    try {
...
async function cmdLs(args) {
    ...
    if (!rawPath)
        throw new CliError('ls requires <path>', 2);
...
async function cmdRead(args) {
    const rawPath = args.positionals[0];
    if (!rawPath)
        throw new CliError('read requires <path>', 2);
    const validation = await validatePath(rawPath);
    if (!validation.ok) {
...
async function cmdWrite(args) {
    const rawPath = args.positionals[0];
    if (!rawPath)
        throw new CliError('write requires <path>', 2);
    if (args.content === null)
        throw new CliError('write requires --content <text>', 2);
    const validation = await validatePath(rawPath);
    if (!validation.ok) {
...
async function cmdMkdir(args) {
    const rawPath = args.positionals[0];
    if (!rawPath)
        throw new CliError('mkdir requires <path>', 2);
    const validation = await validatePath(rawPath);
    if (!validation.ok) {
...
async function cmdRm(args) {
    const rawPath = args.positionals[0];
    if (!rawPath)
        throw new CliError('rm requires <path>', 2);
    const validation = await validatePath(rawPath);
    if (!validation.ok) {
...
async function cmdMv(args) {
    ...
    if (!rawSrc || !rawDst)
        throw new CliError('mv requires <src> <dst>', 2);
...
async function cmdCp(args) {
    ...
    const rawDst = args.positionals[1];
    if (!rawSrc || !rawDst)
        throw new CliError('cp requires <src> <dst>', 2);
    const srcValidation = await validatePath(rawSrc);
    if (!srcValidation.ok) {
...
async function cmdStat(args) {
    const rawPath = args.positionals[0];
    if (!rawPath)
        throw new CliError('stat requires <path>', 2);
    const validation = await validatePath(rawPath);
    if (!validation.ok) {
