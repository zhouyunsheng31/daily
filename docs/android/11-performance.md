...
| 端侧 pi 进程（D15，M0-3 实测入表） | **RSS 常态 ≤ 300MB**（含会话上下文，10 会话内）；首 token 延迟 ≤ 5s（中端机、用户 Key 网络正常） | `dumpsys meminfo` / 进程 RSS 埋点 + harness 内自报；**M0-3 实测（魅族 Lucky 08，2026-08-15，见 perf-reports/m0-3-onside-pi-2026-08-15.md）：冷启动 2.7s、首 token 均值 2.9s、RSS 稳定 140–142MB（10 会话后）、12 轮上下文增长平坦** |
| 本地 AI 冷启动（D15） | proot 拉起 → 可对话 ≤ 10s（首启 rootfs 下载另计，需引导进度） | 埋点：harness ready 事件；**实测 2.7s ✅**；rootfs 155MB 下载 4.3MB/s ≈ 36s（引导进度 UI 排 M2-8） |
