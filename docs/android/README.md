...
| D21 | **对话页保持原生 Compose，不整体 WebView 化（2026-08-16 用户拍板）**：对话页是最高频交互页（流式渲染/长列表/IME/端侧 AI 桥直连），WebView 化会砸 D11 性能红线（60/120fps、RSS≤300MB、冷启动）。**但 D20 机制保留"对话页可被 HTML 对话 App 覆盖"**（用户/AI 可装 HTML 对话包替换默认对话页，性能自负）；桌面↔对话横滑的跟手问题**不靠 WebView 化解决**，由 M1-4 **边缘手势让渡**（WebView 边缘触摸位移实时转发给 Pager 动画）实现，接近原生跟手 | 10 §2.1、12 M1-4 |
