...
| **M1-2** | 系统模板消费与 JSBridge 完备 | 🔶 进行中 | 直接加载 Web 端对话页与桌面模板，打通全部 SDK 桥接 |
| **M1-3** | App 管理与版本切换 | ⬜ 未开始 | 打开 App、版本时间线回滚、删除同步 |
| **M1-4** | 本地包缓存与文件同步 | ⬜ 未开始 | 对接服务端 `/webos/api/files/*` 与本地存储 |
| **M1-5** | 性能与启动优化 | ⬜ 未开始 | 预热池、冷启动 < 1s、滑动 60/120fps |
