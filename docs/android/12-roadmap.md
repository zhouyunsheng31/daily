...
| # | 任务 | 验收标准 |
|---|---|---|
| M1-1 | 四大页面完整实现（对话/桌面/商店浏览/我的）按 10 篇规格；空态/错误/加载全套；**含 §0 沉浸式改造（去 Tab 栏/系统栏沉浸，导航改手势模型——候选经用户选定）** | 10 §6 可用性清单逐条通过 + §0 沉浸验收（edge-to-edge、无应用感系统栏） |
...
| M1-4 | 桌面：网格模式 + 长按编辑 + 壁纸 + **桌宠层（应用内，A 形态唯一落点）**：桌面页共享 canvas WebView 挂载点 + 平台默认极简桌宠（默认包）+ **pet-layer 最小包加载提前**（包声明 kind=pet-layer → 加载进桌宠层，AI 生成桌宠包即时可用）；桌宠内容 100% AI 包化，宿主只做容器；**D18 启动器体验：多页网格 + 拖图标边缘翻页 + 叠放建文件夹 + 桌面↔对话手势滑入滑出（宿主层），手势边界与手感预算进设计文档** | J5 实测；50 图标流畅（11 §2）；AI 生成的 pet-layer 包安装后出现在桌宠层并可回滚（D3 默认包可覆盖）；**10 §0 启动器手势逐条实测通过** |
