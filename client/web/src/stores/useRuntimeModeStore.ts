 ...
 *
 * 注意：
 * - isServerOnline 默认 false（保守：先假定离线，等 healthCheck 上报真实状态）
...
export const useRuntimeModeStore = create<RuntimeModeStoreState>((set, get) => ({
  ...
  effectiveMode: computeEffectiveMode(loadPersistedMode(), false),
  isOfflineDowngraded: computeOfflineDowngraded(loadPersistedMode(), false),
  _debounceTimer: null,
