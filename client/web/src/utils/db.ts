import type { PanelData as V1PanelData, AppData, Panel, WidgetInstance, WidgetPosition, AppSettings, DynamicWidgetDef, MusicPlaylist, FocusSession, Task, Habit, HabitCheckin, MoodEntry, CalendarEvent, DrawingStroke, WidgetConnection, QuizSession, Bookmark } from '../types'
  ...
  return withFallback(
    async () => {
      ...
      return dtos.map(dto => ({
        ...
        isCommunity: dto.isCommunity ?? false,
        communityApiUrl: dto.communityApiUrl ?? null,
      }))
    },
    async () => {
      await ensureV2Ready()
...
// S14.1-T2 改造（M8 修复）：接受 options 参数并透传给 dynamicWidgetsApi.getAllDynamicWidgets(options)
export async function getAllDynamicWidgets(options?: { desktop?: boolean }): Promise<DynamicWidgetDef[]> {
  return withFallback(
    async () => {
      const dtos = await dynamicWidgetsApi.getAllDynamicWidgets(options)
      return dtos.map(dto => ({
...
// ========================= Bookmark 持久化 =========================
// 说明：bookmarks 暂无服务端 API，直接使用 IDB 持久化。
