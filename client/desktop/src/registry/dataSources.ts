...
export function registerAllDataSources(): void {
  ...
  registerDataSource({
    ...
    aiReadable: true,
    aiWritable: false,
    schema: { id: 'ID', panelId: '面板ID', focusTimerWidgetId: '专注组件ID', taskId: '任务ID', label: '标签', startedAt: '开始时间', endedAt: '结束时间', duration: '时长', mode: '模式' },
  })
  ...
  registerDataSource({
    storeName: 'quizSessions',
    displayName: 'LaTeX 答题会话',
    category: 'study',
    aiReadable: true,
    aiWritable: false,
    schema: {
      id: '会话ID',
      panelId: '面板ID',
      latexQuizWidgetId: '组件ID',
      category: '分类(algebra/geometry/calculus/trig)',
      questionIds: '题目ID列表',
      correctCount: '正确数',
      totalCount: '总题数',
      startedAt: '开始时间',
  ...
  registerDataSource({
    ...
    aiReadable: true,
    aiWritable: true,
    schema: { widgetId: '组件ID', tracks: '曲目列表', currentTrackIndex: '当前曲目索引', isPlaying: '是否播放中' },
  })
