...
// ---------- 16. webTabs ----------
async function adaptWebTabs(): Promise<SearchableRecord[]> {
  try {
    const tabs = await getAllWebTabs()
    return tabs.map((t): SearchableRecord => ({
      id: String(t.id),
      storeId: 'webTabs',
      type: 'webTab',
...
export function buildAllAdapters(): SearchAdapter[] {
  return [
    ...
    namedAdapter(adaptWebTabs, 'webTabs'),
