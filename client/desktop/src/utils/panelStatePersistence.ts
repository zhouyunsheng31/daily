    ...
    if (widget.widgetType === 'webPage' || widget.widgetType === 'webview') {
      ...
      if (webview) {
        try {
          ...
          const scrollY = await webview.executeJavaScript('window.scrollY', false)
...
export async function restoreWebviewScrollY(widgetId: string, scrollY: number): Promise<void> {
  const webview = browserToolBridge.getWebview(widgetId)
  if (!webview) {
    console.warn(`[panelStatePersistence] Webview not found for widget: ${widgetId}`)
    return
  }
  try {
    await webview.executeJavaScript(`window.scrollTo(0, ${Math.max(0, scrollY)})`, false)
