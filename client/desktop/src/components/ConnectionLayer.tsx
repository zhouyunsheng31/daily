...
import type { WidgetPosition, WidgetConnection, WidgetInstance } from '../types'
...
export function ConnectionLayer({ panelId }: ConnectionLayerProps) {
  ...
  const widgets = useAppStore(s => s.panelWidgets[panelId] ?? EMPTY_WIDGETS)
  ...
  // 缓存 widgetId → widgetType 查找（用于过滤 AI 助手相关连线）
  const widgetTypeMap = useMemo(() => {
    const map = new Map<string, string>()
    for (const w of widgets) {
      map.set(w.widgetId, w.widgetType)
    }
    return map
  }, [widgets])

  // 过滤掉引用不存在 widget 的连线；隐藏 AI 助手时过滤相关连线
  const validConnections = useMemo(() => {
    return connections.filter(c => {
      if (!positionMap.has(c.source.widgetId) || !positionMap.has(c.target.widgetId)) return false
      if (hideAIAssistant) {
        const sourceType = widgetTypeMap.get(c.source.widgetId)
        const targetType = widgetTypeMap.get(c.target.widgetId)
        if (sourceType === 'aiAssistant' || targetType === 'aiAssistant') return false
      }
      return true
    })
  }, [connections, positionMap, widgetTypeMap, hideAIAssistant])
