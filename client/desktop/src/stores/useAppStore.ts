...
import type { Panel, PanelSettings, WidgetInstance, WidgetPosition, AppSettings, SaveStatus, DynamicWidgetDef, CanvasTransform, DrawingStroke, DrawingStrokeType, DrawingStyle, WidgetConnection, CanvasMode, WebviewWidgetState, WebTab, Bookmark, ConflictInfo, MainView, FavoriteEntry, FavoriteGroup, HomeTemplateType, AccessibilitySettings } from '../types'
import { DEFAULT_APPEARANCE, DEFAULT_BEHAVIOR, DEFAULT_DRAWING_STYLE, DEFAULT_ACCESSIBILITY } from '../types'
...
// v10: calculateFreePosition 辅助函数 — 为新 widget 计算空闲位置（避免重叠）
function calculateFreePosition(
  existingPositions: WidgetPosition[],
  size: { w: number; h: number },
): { x: number; y: number; w: number; h: number } {
  const step = 50
  const maxSearch = 2000
  for (let y = 0; y < maxSearch; y += step) {
    for (let x = 0; x < maxSearch; x += step) {
      const candidate = { x, y, w: size.w, h: size.h }
      const overlap = existingPositions.some(p =>
        x < p.x + p.w &&
        x + candidate.w > p.x &&
        y < p.y + p.h &&
        y + candidate.h > p.y
...
interface AppState {
  ...
  addPanel: (name: string, options?: { skipPrimaryAI?: boolean }) => Promise<string>
  ...
  // Phase 7 批次4 任务7（spec 6.2.3 节）：无障碍设置更新（与 appearance/behavior 同级）
  ...
  // WebTab 管理（spec 5.2 节，网页标签独立于画布面板）
  webTabs: WebTab[]
  activeWebTabId: string | null
  addWebTab: (url?: string) => Promise<string>
  ...
  // Phase 7 批次3 任务4：拖拽批量重排 + 最近使用 touch + 分组模式
  reorderFavorites: (items: { id: string; sortIndex: number }[]) => Promise<void>
  touchFavorite: (id: string) => Promise<void>
  favoriteGroupBy: 'none' | 'panel' | 'type' | 'custom'
  setFavoriteGroupBy: (mode: 'none' | 'panel' | 'type' | 'custom') => void

  // ========== Phase 7 批次3 任务6：组件搜索浮层状态（迁移 App.tsx Ctrl+F 本地 state） ==========
  showWidgetSearch: boolean
  setShowWidgetSearch: (open: boolean) => void

  // ========== Phase 7 批次6 任务5：主页模板（极简/标准/丰富） ==========
  homeTemplate: HomeTemplateType
  setHomeTemplate: (template: HomeTemplateType) => void
...
// webTabs / bookmarks 防抖持久化（500ms 合并高频更新，如 URL 导航、标题更新）
const debouncedWebTabsSave = createDebouncedSave(
  (tabs: WebTab[]) => {
    saveWebTabs(tabs).catch((err) => {
      console.error('[Persist] webTabs save failed:', err)
    })
  },
  500
)
...
// Phase 7 批次3 任务4：收藏本地 meta 降级持久化（spec 5.1.2 降级策略）
// 服务器未升级时 orderIndex/groupName/lastUsedAt 仅存 localStorage
// key: favorites_local_meta，value: Record<favoriteId, LocalFavoriteMeta>
const FAVORITE_LOCAL_META_STORAGE_KEY = 'favorites_local_meta'

export interface LocalFavoriteMeta {
  sortIndex?: number
  groupId?: string
  groupName?: string
  lastUsedAt?: number
...
function loadFavoritesLocalMeta(): Record<string, LocalFavoriteMeta> {
  try {
    const raw = localStorage.getItem(FAVORITE_LOCAL_META_STORAGE_KEY)
    if (!raw) return {}
    const parsed = JSON.parse(raw) as Record<string, LocalFavoriteMeta>
...
function saveFavoritesLocalMeta(meta: Record<string, LocalFavoriteMeta>): void {
  try {
    localStorage.setItem(FAVORITE_LOCAL_META_STORAGE_KEY, JSON.stringify(meta))
  } catch { /* ignore quota / privacy errors */ }
}

function mergeFavoriteWithLocalMeta(
  fav: FavoriteEntry,
  meta: LocalFavoriteMeta | undefined,
): FavoriteEntry {
  if (!meta) return fav
  return {
    ...fav,
    sortIndex: fav.sortIndex ?? meta.sortIndex,
    groupId: fav.groupId ?? meta.groupId,
    groupName: fav.groupName ?? meta.groupName,
    lastUsedAt: fav.lastUsedAt ?? meta.lastUsedAt,
    updatedAt: fav.updatedAt ?? meta.updatedAt,
  }
}
...
// Phase 7 批次6 任务5：主页模板持久化（key: ld_home_template）
// 独立于 settings，避免改动 AppSettings/API 层；默认 'standard'
const HOME_TEMPLATE_STORAGE_KEY = 'ld_home_template'
const DEFAULT_HOME_TEMPLATE: HomeTemplateType = 'standard'

function loadHomeTemplateFromStorage(): HomeTemplateType {
  try {
    const raw = localStorage.getItem(HOME_TEMPLATE_STORAGE_KEY)
    if (raw === 'minimal' || raw === 'standard' || raw === 'rich') return raw
    return DEFAULT_HOME_TEMPLATE
  } catch {
    return DEFAULT_HOME_TEMPLATE
  }
}

function saveHomeTemplateToStorage(template: HomeTemplateType): void {
  try {
    localStorage.setItem(HOME_TEMPLATE_STORAGE_KEY, template)
  } catch { /* ignore quota / privacy errors */ }
}
...
// webTabs / bookmarks 持久化 subscribe 跟踪
let _unsubPersistCollections: (() => void) | null = null
let _prevWebTabs: WebTab[] | null = null   // null 表示尚未从 IDB 加载，避免初始化时触发保存
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  initialize: async () => {
    ...
    try {
      ...
      if (backend === 'api') {
        let [panelsData, activeId, settingsData] = await Promise.all([
        ...
        set({
          panels: panels.map(p => ({
            ...
            canvasTransform: (p.id === activeId ? canvasTransform : p.canvasTransform) as CanvasTransform | undefined,
          })),
          activePanelId: activeId ?? panels[0]?.id ?? null,
          panelWidgets,
          panelPositions,
          ...
          settings: {
            ...
            accessibility: { ...DEFAULT_ACCESSIBILITY, ...((settingsData.accessibility as AccessibilitySettings | undefined) || {}) },
          } as AppSettings,
          initialized: true,
        ...
        const activeId = data.activePanelId ?? panels[0]?.id ?? null
        const activePanel = panels.find(p => p.id === activeId)
        // 优先从 sessionStorage 恢复 canvasTransform（同步，刷新后仍存在）
        let canvasTransform = activePanel?.canvasTransform ?? { x: 0, y: 0, zoom: 1 }
        if (activeId) {
    ...
    // 加载 webTabs / bookmarks（独立于主初始化，失败不影响 panels/widgets）
    try {
      const [savedTabs, savedBookmarks] = await Promise.all([
        getWebTabs(),
        getBookmarks(),
      ])
      // 初始化 _prev 引用为已加载值，避免 subscribe 首次触发误保存
      _prevWebTabs = savedTabs
      _prevBookmarks = savedBookmarks
      set({ webTabs: savedTabs, bookmarks: savedBookmarks })
    } catch (err) {
      console.error('[useAppStore] Failed to load webTabs/bookmarks:', err)
      _prevWebTabs = []
      _prevBookmarks = []
    }

    // 设置 subscribe 监听 webTabs / bookmarks 变化，自动防抖保存到 IDB
    ...
    _unsubPersistCollections = useAppStore.subscribe((state) => {
      // null 表示尚未初始化完成，跳过保存
      if (_prevWebTabs !== null && state.webTabs !== _prevWebTabs) {
        _prevWebTabs = state.webTabs
        debouncedWebTabsSave.call(state.webTabs)
      }
  ...
  addPanel: async (name: string, options?: { skipPrimaryAI?: boolean }): Promise<string> => {
    ...
    await withFallback(
      async () => { await panelsApi.createPanel({ id: newPanel.id, name: newPanel.name, sortOrder: newPanel.order, settings: newPanel.settings as Record<string, unknown> }) },
      () => savePanel(newPanel),
    )
    await withFallback(
    ...
    // v10: 自动添加主AI助手 widget（除非 skipPrimaryAI，修复 New Issue 3/6）
    if (!options?.skipPrimaryAI) {
      await get().addWidget('aiAssistant', {
        panelId: newPanel.id,
        position: { x: 0, y: 0, w: 400, h: 500 },
        isPrimary: true,
        initialState: {},
      })
    }
  ...
  addPanelFromTemplate: async (templateId: string) => {
    ...
    // v10: 传入 skipPrimaryAI: true，由模板内容决定是否添加主AI助手（修复 New Issue 3）
    const panelId = await get().addPanel(template.name, { skipPrimaryAI: true })

    // 跟踪模板中是否有 aiAssistant widget（修复 v5 New Major 1）
    let hasAIInTemplate = false

    for (const widget of template.widgets) {
      // 模板中第一个 aiAssistant 升级为主AI（避免重复）
      const isPrimaryAI = !hasAIInTemplate && widget.widgetType === 'aiAssistant'
      if (isPrimaryAI) hasAIInTemplate = true
      try {
        await get().addWidget(widget.widgetType, {
          panelId,
          position: widget.position,
          initialState: widget.initialState,
          isPrimary: isPrimaryAI,
        })
      } catch (e) {
    ...
    // 模板没有 aiAssistant widget，补加主AI助手（用 calculateFreePosition 避免重叠，修复 v7 Minor 1）
    if (!hasAIInTemplate) {
      const positions = get().panelPositions[panelId] ?? []
      const position = calculateFreePosition(positions, { w: 400, h: 500 })
      await get().addWidget('aiAssistant', {
        panelId,
        position,
        isPrimary: true,
  ...
  // Phase 7 批次4 任务7（spec 6.2.3 节）：无障碍设置更新
  // 与 appearance/behavior 同级持久化（settings.accessibility），App.tsx 订阅并应用到 document.documentElement
  updateAccessibility: async (partial: Partial<AccessibilitySettings>) => {
    const current = get().settings.accessibility ?? DEFAULT_ACCESSIBILITY
    const newSettings = {
      ...get().settings,
      accessibility: { ...current, ...partial },
    }
    await withFallback(
      () => settingsApi.updateSettings(newSettings as unknown as Record<string, unknown>),
      () => saveSettings(newSettings),
  ...
  destroy: () => {
    ...
    // 清理 webTabs / bookmarks 持久化 subscribe 和防抖保存
    if (_unsubPersistCollections) {
      _unsubPersistCollections()
      _unsubPersistCollections = null
    }
    debouncedWebTabsSave.cancel()
  ...
  toggleHideAIAssistant: () => {
    set(state => ({ hideAIAssistant: !state.hideAIAssistant }))
  },
  ...
  // S5 修复：addWidget 失败时清理空面板；F2 修复：内联查找 widget（findWidgetInstance 未导出）
  convertWidgetToTab: async (widgetId: string) => {
    ...
    const widgetState = widget.state as unknown as WebviewWidgetState
  ...
  convertTabToWidget: async (sourcePanelId: string, targetPanelId?: string) => {
    ...
    // try/catch：addWidget 可能因 5-webview 限制 throw，捕获后重新抛出，让调用方提示用户
    try {
      await state.addWidget('webPage', {
        panelId: targetId,
      ...
      // convertTabToWidget 不创建新面板，只是往现有面板加 widget，无需清理面板
      throw e  // 重新抛出，让调用者处理（如 TabBar 右键菜单的 onClick）
    }
  },
  ...
  // WebTab 管理（通过 IDB 持久化，subscribe 自动保存）
  webTabs: [],
  activeWebTabId: null,
  addWebTab: async (url?: string) => {
    const tabId = uuidv4()
    const now = Date.now()
    const newTab: WebTab = {
      id: tabId,
    ...
    set(state => ({
      webTabs: [...state.webTabs, newTab],
      activeWebTabId: tabId,
    }))
    return tabId
  },
  closeWebTab: async (tabId: string) => {
    set(state => {
      const newTabs = state.webTabs.filter(t => t.id !== tabId)
      const newActiveId = state.activeWebTabId === tabId
        ? newTabs[newTabs.length - 1]?.id ?? null
        : state.activeWebTabId
      return { webTabs: newTabs, activeWebTabId: newActiveId }
    })
  },
  setActiveWebTab: (tabId: string) => {
    set({ activeWebTabId: tabId })
  },
  updateWebTab: (tabId: string, partial: Partial<WebTab>) => {
    set(state => ({
      webTabs: state.webTabs.map(t =>
        t.id === tabId ? { ...t, ...partial, updatedAt: Date.now() } : t
      ),
    }))
  },

  // 主区域显示
  mainView: { type: 'canvas-panel' },
  setMainView: (view: Partial<MainView>) => {
    set(state => ({ mainView: { ...state.mainView, ...view } }))
  },
  ...
  refreshFavorites: async () => {
    try {
      ...
      const localMeta = loadFavoritesLocalMeta()
      const favorites: FavoriteEntry[] = dtos.map(dto => {
        const fav: FavoriteEntry = {
          ...
          // Phase 7 批次3 任务4：服务器升级后的扩展字段（向后兼容，undefined 时由本地 meta 兜底）
        ...
        return mergeFavoriteWithLocalMeta(fav, localMeta[fav.id])
      })
      set({ favorites })
    } catch (err) {
  ...
  removeFavorite: async (favoriteId: string) => {
    ...
    // Phase 7 批次3 任务4：清理本地 meta
    const meta = loadFavoritesLocalMeta()
    if (meta[favoriteId]) {
      delete meta[favoriteId]
      saveFavoritesLocalMeta(meta)
    }
  },
  ...
  removeFavoritesByPanelId: async (panelId: string) => {
    const favIdsToRemove = get().favorites.filter(f => f.panelId === panelId).map(f => f.id)
    await withFallback(
    ...
    // Phase 7 批次3 任务4：批量清理本地 meta
    if (favIdsToRemove.length > 0) {
      const meta = loadFavoritesLocalMeta()
      let changed = false
      for (const fid of favIdsToRemove) {
        if (meta[fid]) { delete meta[fid]; changed = true }
      }
      if (changed) saveFavoritesLocalMeta(meta)
    }
  },
  ...
  updateFavoriteGroupAssignment: async (id, groupId?, groupName?) => {
    const prevFavorites = get().favorites
    const prevMeta = loadFavoritesLocalMeta()
    const updatedFavorites = prevFavorites.map(f => {
      if (f.id !== id) return f
    ...
    set({ favorites: updatedFavorites })
    // 同步写本地 meta：服务器未升级时这是唯一持久化途径
    const meta = { ...prevMeta }
    if (groupId === undefined) {
      if (meta[id]) {
        delete meta[id].groupId
        delete meta[id].groupName
        meta[id] = { ...meta[id], updatedAt: Date.now() }
      }
    } else {
      meta[id] = { ...meta[id], groupId, groupName: groupName ?? meta[id]?.groupName, updatedAt: Date.now() }
    }
    saveFavoritesLocalMeta(meta)
    try {
      await withFallback(
        () => favoritesApi.updateFavoriteGroup(id, groupId, groupName),
        async () => { /* 降级：本地 meta 已持久化 */ },
      ...
      set({ favorites: prevFavorites })
      saveFavoritesLocalMeta(prevMeta)
    }
  },
  // Phase 7 批次3 任务4：批量重排（拖拽场景）+ touchFavorite（更新 lastUsedAt）
  reorderFavorites: async (items) => {
    // 乐观更新：先把 sortIndex 应用到本地 favorites
    const prevFavorites = get().favorites
    const prevMeta = loadFavoritesLocalMeta()
    const sortIndexMap = new Map(items.map(it => [it.id, it.sortIndex] as const))
    const updatedFavorites = prevFavorites.map(f =>
    ...
    for (const it of items) {
      meta[it.id] = { ...meta[it.id], sortIndex: it.sortIndex, updatedAt: now }
    }
    saveFavoritesLocalMeta(meta)
    try {
      await withFallback(
        () => favoritesApi.reorderFavorites(items),
        async () => { /* 降级：本地 meta 已持久化 */ },
        { operation: 'update', entityType: 'favoriteReorder', entityId: 'batch', payload: { items } },
      )
    } catch (err) {
      console.error('[useAppStore] reorderFavorites failed, rolling back:', err)
      set({ favorites: prevFavorites })
      saveFavoritesLocalMeta(prevMeta)
      throw err  // 让 UI 层捕获后 Toast 提示
    }
  },
  touchFavorite: async (id) => {
    const now = Date.now()
    // 乐观更新本地 favorites 的 lastUsedAt
    const prevFavorites = get().favorites
    const updatedFavorites = prevFavorites.map(f =>
      f.id === id ? { ...f, lastUsedAt: now } : f
    )
    set({ favorites: updatedFavorites })
    // 同步写本地 meta：服务器未升级时这是唯一持久化途径
    const prevMeta = loadFavoritesLocalMeta()
    const meta = { ...prevMeta, [id]: { ...prevMeta[id], lastUsedAt: now, updatedAt: now } }
    saveFavoritesLocalMeta(meta)
    try {
      await withFallback(
        () => favoritesApi.touchFavorite(id),
        async () => { /* 降级：本地 meta 已持久化 */ },
        { operation: 'update', entityType: 'favoriteTouch', entityId: id, payload: {} },
      )
    } catch (err) {
      console.error('[useAppStore] touchFavorite failed:', err)
      // touch 失败不回滚（非关键路径），保留本地乐观更新
    }
  },
  // Phase 7 批次3 任务4：分组模式（none/panel/type/custom）
  favoriteGroupBy: 'none',
  setFavoriteGroupBy: (mode) => {
    set({ favoriteGroupBy: mode })
  },
  ...
  // ========== Phase 7 批次6 任务5：主页模板（极简/标准/丰富） ==========
  // 初始值从 localStorage 读取，默认 'standard'
  homeTemplate: loadHomeTemplateFromStorage(),
  setHomeTemplate: (template) => {
    saveHomeTemplateToStorage(template)
    set({ homeTemplate: template })
  },
